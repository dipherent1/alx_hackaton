...
# Define MCP tools
@mcp.tool()
def get_weather(location: str, timezone_offset: float = 0) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return get_weather_forecast(location, timezone_offset)
    except Exception as e:
        print(f"Error getting weather for {location}: {str(e)}")
        return {"error": f"Could not get weather for {location}: {str(e)}"}

@mcp.tool()
def get_current_weather(location: str, timezone_offset: float = 0) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        full_weather = get_weather(location, timezone_offset)
        ...
        print(f"Error getting current weather for {location}: {str(e)}")
        return {"error": f"Could not get current weather for {location}: {str(e)}"}
